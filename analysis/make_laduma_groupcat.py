...
for ii, z_i in enumerate(zedges[0:]):
    ...
    pg3obj=pg3.prob_g3groupfinder_luminosity(df.RAall, df.DECall, speed_of_light*df.bestoverallredshift, speed_of_light*df.bestoverallredshifterr,\
    ...
    grpra, grpdec, grpz, _ = pg3obj.get_group_centers(False)
    df.loc[:,'pg3grpradeg'] = grpra
    df.loc[:,'pg3grpdedeg'] = grpdec
    df.loc[:,'pg3grpz'] = grpz
    df.to_csv(fname.replace('.hdf5','_pg3.csv'),index=False)
